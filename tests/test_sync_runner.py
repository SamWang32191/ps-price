...
@pytest.mark.django_db
def test_run_catalog_and_snapshot_sync_fetches_each_catalog_page_once(monkeypatch):
    calls: dict[str, int] = {"catalog": 0}
    ...
    class FakeClient:
        ...
        def fetch_catalog_page(self, page: int):
            calls["catalog"] += 1
            return f"https://store.playstation.com/zh-hant-tw/category/test/{page}", "catalog-page-html"
    ...
    monkeypatch.setattr(sync_runner, "parse_product_detail", lambda *args, **kwargs: None)
    monkeypatch.setattr(sync_runner, "normalize_product_detail_price", lambda *args, **kwargs: None)
